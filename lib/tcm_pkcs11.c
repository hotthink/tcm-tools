 ...
 * Global variables
 */
char *                g_pszSoLib = TCM_OPENCRYPTOKI_SO;
...
CK_SLOT_ID     g_tSlotId;		// Slot ID of the TCM token
CK_TOKEN_INFO  g_tToken;		// TCM token information
 ...
 *   for the TCM token and "opening" it if it is found.
 */
CK_RV
openToken( char *a_pszTokenLabel ) {
	...
	// Iterate through the slots looking for the TCM token
	for ( i = 0; i < ulSlots; i++ ) {
	...
	if ( !g_bTokenOpen ) {
		logError( _("PKCS#11 TCM Token is not present\n") );
		rv = CKR_TOKEN_NOT_PRESENT;
	}
...
findObjects( CK_SESSION_HANDLE  a_hSession,
	...
	*a_pulObjCount = 0;
	...
	do {
		...
		rv = g_pFcnList->C_FindObjects( a_hSession, phObjList + ulCurCount, TCM_FIND_MAX, &ulCount );
...
/*
 * isTokenInitialized
 *   Returns an indicator as to whether the TCM token has been initialized.
 */
BOOL
isTokenInitialized( ) {
...
/*
 * getMinPinLen
 *   Returns the the minimum PIN length that the TCM token accepts.
 */
int
getMinPinLen( ) {
...
/*
 * getMaxPinLen
 *   Returns the the maximum PIN length that the TCM token accepts.
 */
int
getMaxPinLen( ) {
