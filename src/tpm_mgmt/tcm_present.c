...
static struct physFlag flags[] = {
	{N_("Command Enable"), TSM_TCMSTATUS_PHYSPRES_CMDENABLE},
	{N_("Hardware Enable"), TSM_TCMSTATUS_PHYSPRES_HWENABLE},
	{N_("Lifetime Lock"), TSM_TCMSTATUS_PHYSPRES_LIFETIMELOCK},
	{N_("Physical Presence"), TSM_TCMSTATUS_PHYSPRESENCE},
	{N_("Lock"), TSM_TCMSTATUS_PHYSPRES_LOCK},
	{0, 0, 0, 0}
};
static BOOL bCheck = FALSE;
static BOOL bChangeRequested = FALSE;
static BOOL bYes = FALSE;
static TSM_BOOL bValue;
static BOOL isWellKnown = FALSE;
static BYTE well_known[] = TSM_WELL_KNOWN_SECRET;
...
static void help(const char *aCmd)
{
	...
	logCmdOption("--lock",
		     _("Lock TCM presence assertion into specified state."));
	logCmdOption("--enable-cmd",
		     _("Allow TCM to accept Physical Presence Command."));
	logCmdOption("--disable-cmd",
		     _("Disallow TCM to accept Physical Presence Command."));
	logCmdOption("--enable-hw",
		     _("Allow TCM to accept Hardware Physical Presence."));
	logCmdOption("--disable-hw",
		     _("Disallow TCM to accept Hardware Physical Presence."));
	logCmdOption("--set-lifetime-lock",
		     _("Prevent further modification of TCM Physical Presence\n\t\tCommand and Hardware Enablement states.\n\t\tTHIS ACTION IS PERMANENT AND CAN NEVER BE UNDONE."));
	logCmdOption("-y, --yes",
...
static BOOL confirmLifeLock(TSM_HCONTEXT hContext, TSM_HTCM hTpm)
{
	...
	if (tpmGetStatus(hTpm, flags[cmdEnable].property, &bCmd) !=
		...
		if (isTpmOwned(hContext)) {
			...
			goto give_vals;
		      ...
		      warn:
			logMsg
			    (_("Unable to programatically determine the current setting of TCM Physcial Presence Command Enable and Hardware Enable states.  Make sure you are aware of and comfortable with the current states.\n"));
		}
	} else {
	      give_vals:
...
/*
 * Affect: Toggle TCM presence states
