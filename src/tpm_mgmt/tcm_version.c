...
int cmdVersion(const char *a_szCmd)
{
...
#ifdef TSM_LIB_IS_12
	{
		 ...
		 * If we're on a 1.1 TCM, it'd throw an error.
		...
		uiResult = getCapability(hTpm, TSM_TCMCAP_VERSION_VAL, 0, NULL, &uiResultLen,
					 &pResult);

		// Restore output to stderr.
		errbuf = end_capture_stderr(olderr);

		// Don't print errors due to "Bad Mode", that just means we're on a 1.1 TCM.
		if (uiResult == TCM_E_BAD_MODE) {
		        free(errbuf);
...
print_cap_version:
#endif
	if (getCapability(hTpm, TSM_TCMCAP_VERSION, 0, NULL, &uiResultLen,
			  &pResult) != TSM_SUCCESS)
		goto out_close;
	logMsg(_("  TCM Version:         "));
	logHex(uiResultLen, pResult);

	uiSubCap = TSM_TCMCAP_PROP_MANUFACTURER;
	pSubCap = (BYTE *) & uiSubCap;
	if (getCapability(hTpm, TSM_TCMCAP_PROPERTY, sizeof(uiSubCap),
			  pSubCap, &uiResultLen, &pResult) != TSM_SUCCESS)
		goto out_close;
