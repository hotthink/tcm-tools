...
TSM_RESULT getNVDataPublic(TSM_HTCM hTpm, TCM_NV_INDEX nvindex,
                           TCM_NV_DATA_PUBLIC **pub)
{
	TSM_RESULT res;
	UINT32 ulResultLen;
	BYTE *pResult;
	...
	res = getCapability(hTpm, TSM_TCMCAP_NV_INDEX, sizeof(UINT32),
			    (BYTE *)&nvindex, &ulResultLen, &pResult);

	if (res != TSM_SUCCESS)
		return res;

	*pub = calloc(1, sizeof(TCM_NV_DATA_PUBLIC));
	if (*pub == NULL) {
		res = TSM_E_OUTOFMEMORY;
		goto err_exit;
	}
...
void freeNVDataPublic(TCM_NV_DATA_PUBLIC *pub)
{
	if (!pub)
		return;
