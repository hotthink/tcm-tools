...
static void help(const char *aCmd)
{
	...
	logCmdOption("-g, --original_password_unicode", _("Use TSM UNICODE encoding for original password to comply with applications using TSM popup boxes"));
	logCmdOption("-n, --new_password_unicode", _("Use TSM UNICODE encoding for new password to comply with applications using TSM popup boxes"));
	logCmdOption("-z, --well-known", _("Change password to a new one when current owner password is a secret of all zeros (32 bytes of zeros). It must be specified which password (owner, SRK or both) to change"));
	logCmdOption("-r, --set-well-known", _("Change password to a secret of all zeros (32 bytes of zeros). It must be specified which password (owner, SRK or both) to change"));
}
...
static TSM_RESULT
tpmChangeAuth(TSM_HCONTEXT aObjToChange,
	      TSM_HOBJECT aParent, TSM_HPOLICY aNewPolicy)
{
	TSM_RESULT result =
...
int main(int argc, char **argv)
{
	...
	char *passwd = NULL;
	int pswd_len;
	TSM_HPOLICY hTpmPolicy, hNewPolicy;
	TSM_HTCM hTpm;
	TSM_HTCM hSrk;
	...
	if (wellKnown) {
		...
		pswd_len = TCPA_SM3_256_HASH_LEN;
	} else {
	...
	if (policySetSecret(hTpmPolicy, pswd_len, (BYTE *)passwd) != TSM_SUCCESS)
		goto out_close;
	...
	do {
		if (auths[i].change) {
			...
			if (policySetSecret
			    (hNewPolicy, pswd_len,
			     (BYTE *)passwd) != TSM_SUCCESS)
				goto out_close;

			if (i == owner) {
				if (tpmChangeAuth(hTpm, NULL_HOBJECT, hNewPolicy) != TSM_SUCCESS)
					goto out_close;
			} else if (i == srk) {
				if (keyLoadKeyByUUID
				    (hContext, TSM_PS_TYPE_SYSTEM,
				     SRK_UUID, &hSrk) != TSM_SUCCESS)
					goto out_close;
				if (tpmChangeAuth(hSrk, hTpm, hNewPolicy) != TSM_SUCCESS)
					goto out_close;
			}
