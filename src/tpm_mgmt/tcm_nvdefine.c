...
static void help(const char* aCmd)
{
	...
	logCmdOption("-y, --owner-well-known",
		     _("Use 32 bytes of zeros (TSM_WELL_KNOWN_SECRET) as the "
		       "TCM owner secret"));
...
int
parseNVPermsFile(FILE *f, TSM_HCONTEXT *hContext, TSM_HNVSTORE *nvObject,
		 TSM_HPCRS *hPcrsRead, TSM_HPCRS *hPcrsWrite)
{
	UINT32 pcrSize;
	char rw;
	unsigned int pcr, n;
	...
	while (!feof(f)) {
		...
		if (rw == 'r') {
			if (*hPcrsRead == NULL_HPCRS)
				if (contextCreateObject(*hContext, TSM_OBJECT_TYPE_PCRS,
							TSM_PCRS_STRUCT_INFO_SHORT,
							hPcrsRead) != TSM_SUCCESS)
					goto out;
...
int main(int argc, char **argv)
{
	...
	int opswd_len = -1;
	int dpswd_len = -1;
	TSM_HPCRS hPcrsRead = 0, hPcrsWrite = 0;
	struct option hOpts[] = {
	...
	TSM_FLAG initFlag = TSM_PCRS_STRUCT_INFO_SHORT;
	UINT32 localityValue = TCM_LOC_ZERO | TCM_LOC_ONE | TCM_LOC_TWO |
			       TCM_LOC_THREE | TCM_LOC_FOUR;
	...
	if (nvperm == 0 &&
	    (UINT32)nvindex != TCM_NV_INDEX_LOCK &&
	    (UINT32)nvindex != TCM_NV_INDEX0) {
	...
	if (Tspi_SetAttribUint32(nvObject,
				 TSM_TSPATTRIB_NV_PERMISSIONS,
				 0,
				 nvperm) != TSM_SUCCESS)
		goto out_close_obj;

	if (Tspi_SetAttribUint32(nvObject,
				 TSM_TSPATTRIB_NV_DATASIZE,
				 0,
				 nvsize) != TSM_SUCCESS)
		goto out_close_obj;
	...
	if (hPcrsRead)
		if (pcrcompositeSetPcrLocality(hPcrsRead, localityValue) != TSM_SUCCESS)
			goto out_close;

	if (hPcrsWrite)
		if (pcrcompositeSetPcrLocality(hPcrsWrite, localityValue) != TSM_SUCCESS)
			goto out_close;

	if (NVDefineSpace(nvObject, hPcrsRead, hPcrsWrite) != TSM_SUCCESS)
		goto out_close;
