...
static TSM_HPCRS hPcrs = NULL_HPCRS;
...
int main(int argc, char **argv)
{
	...
	UINT32 sealKeyLen;
	BYTE *sealKey;
	TSM_FLAG keyFlags = TSM_KEY_TYPE_STORAGE | TSM_KEY_SIZE_2048 |
	    TSM_KEY_VOLATILE | TSM_KEY_AUTHORIZATION |
	    TSM_KEY_NOT_MIGRATABLE;
	TSM_HPOLICY hSrkPolicy;
	char *passwd = NULL;
	int pswd_len;
	...
	if (selectedPcrsLen) {
...
#ifdef TSM_LIB_IS_12
				initFlag |= TSM_PCRS_STRUCT_INFO_LONG;
#else
				logError(_("This version of %s was compiled for a v1.1 TSM, which "
		...
		if (contextCreateObject(hContext, TSM_OBJECT_TYPE_PCRS, initFlag,
					&hPcrs) != TSM_SUCCESS)
			goto out_close;

		for (i = 0; i < selectedPcrsLen; i++) {
			if (tpmPcrRead(hTpm, selectedPcrs[i], &pcrSize, &pcrValue) != TSM_SUCCESS)
				goto out_close;
		...
		if (initFlag) {
			UINT32 localityValue =
				TCM_LOC_ZERO | TCM_LOC_ONE | TCM_LOC_TWO | TCM_LOC_THREE |
				TCM_LOC_FOUR;

			if (pcrcompositeSetPcrLocality(hPcrs, localityValue) != TSM_SUCCESS)
				goto out_close;
		}
#endif
	}
	...
	/* Build an RSA key object that will be created by the TCM
	...
	if (keyCreateKey(hKey, hSrk, NULL_HPCRS) != TSM_SUCCESS)
		goto out_close;
	...
	if (contextCreateObject
	    (hContext, TSM_OBJECT_TYPE_ENCDATA, TSM_ENCDATA_SEAL,
	...
	if (policyAssign(hPolicy, hEncdata) != TSM_SUCCESS)
		goto out_close;

	/* Encrypt and seal the symmetric key */
	if (dataSeal
	    (hEncdata, hKey, EVP_CIPHER_key_length(EVP_aes_256_cbc()),
	     randKey, hPcrs) != TSM_SUCCESS)
		goto out_close;

	if (getAttribData(hEncdata, TSM_TSPATTRIB_ENCDATA_BLOB,
			  TSM_TSPATTRIB_ENCDATABLOB_BLOB, &encLen,
			  &encKey) != TSM_SUCCESS)
		goto out_close;

	if (getAttribData
	    (hKey, TSM_TSPATTRIB_KEY_BLOB, TSM_TSPATTRIB_KEYBLOB_BLOB,
	     &sealKeyLen, &sealKey) != TSM_SUCCESS)
		goto out_close;
	...
	BIO_puts(bdata, TCMSEAL_HDR_STRING);

	/* Sealing key used on the TCM */
	BIO_puts(bdata, TCMSEAL_TSM_STRING);
	...
	BIO_puts(bdata, TCMSEAL_EVP_STRING);
	BIO_puts(bdata, TCMSEAL_KEYTYPE_SYM);
	BIO_puts(bdata, TCMSEAL_CIPHER_AES256CBC);
	bdata = BIO_push(b64, bdata);
	...
	/* Encrypted Data */
	BIO_puts(bdata, TCMSEAL_ENC_STRING); 
	bdata = BIO_push(b64, bdata);
	...
	EVP_EncryptInit(ctx, EVP_aes_256_cbc(), randKey, (unsigned char *)TCMSEAL_IV);
	...
	BIO_puts( bdata, TCMSEAL_FTR_STRING);
