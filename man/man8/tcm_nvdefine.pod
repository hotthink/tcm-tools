...
Use TSM UNICODE encoding for passwords to comply with applications
using TSM popup boxes

=item B<-y, --owner-well-known>

Use a secret of all zeros (32 bytes of zeros) as the owner's secret.

=item B<-z, --area-well-known>

Use a secret of all zeros (32 bytes of zeros) as the NVRAM area's secret.
...
A write to index 0 locks the NVRAM area until the next TCM_Startup(ST_CLEAR)

=item B<READ_STCLEAR>

A read with size 0 on the same index prevents further reading until the next TCM_Startup(ST_CLEAR)

=item B<WRITE_STCLEAR>

A write with size 0 to the same index prevents further writing until the next TCM_Startup(ST_CLEAR)
