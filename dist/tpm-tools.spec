...
Version:        %{version}
Release:        %{release}
Summary:        Management tools for the TCM hardware
...
Group's TCM hardware. TCM hardware can create, store and use RSA keys
securely (without ever being exposed in memory), verify a platform's
software state using cryptographic hashes and more.

%package pkcs11
Summary:	Data management tools that use a PKCS#11 interface to the TCM
Group:		Applications/Productivity
Requires:	%{name} = %{version}-%{release}, opencryptoki >= 2.2.4

%description pkcs11
%{name}-pkcs11 is a group of tools that uses the TCM PKCS#11 token
developed in the opencryptoki project.  All data contained in the
PKCS#11 data store is protected by the TCM (keys, certificates, etc.).
...
rm -f $RPM_BUILD_ROOT/%{_libdir}/libtcm_unseal.la
...
#%attr(755, root, root) %{_libdir}/libtcm_unseal.a
%attr(755, root, root) %{_libdir}/libtcm_unseal.so.?.?.?
%{_libdir}/libtcm_unseal.so.0
%{_mandir}/man1/tpm_*
%{_mandir}/man8/tpm_*
