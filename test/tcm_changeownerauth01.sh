...
#
#
# Program: tcm_changeownerauth01.sh
# Purpose: Test transitions between plain text, unicode and well-known
# for Owner and SRK passwords, using tcm_changeownerauth from tpm-tools
# package.
...
foreach opt $options {
	...
	if {$opt == "-s"} {
		send_user "\n\n#############################################################\n"
		send_user "####### Testing tcm_changeownerauth for SRK password:########\n"
		send_user "#############################################################\n\n"
	} else {
		send_user "\n\n###############################################################\n"
		send_user "####### Testing tcm_changeownerauth for OWNER password:########\n"
		send_user "###############################################################\n\n"
	}

	# Plaintext -> Unicode
	puts stdout "Test 1) tcm_changeownerauth \"$opt\": Changing password from PLAINTEXT to UNICODE:" 
	spawn tcm_changeownerauth $opt -n -l debug
	sleep 1
	...
	# Unicode -> plaintext
	puts stdout "Test 2) tcm_changeownerauth \"$opt\": Changing password from UNICODE to PLAINTEXT:" 
	spawn tcm_changeownerauth $opt $isU -l debug
	sleep 1
	expect "Enter owner password" {
		exp_send "$ownerPasswd\r"
		if {$opt == "-s"} {
	...
	puts stdout "Test 3) tcm_changeownerauth \"$opt\": Changing password from PLAINTEXT to WELL KNOWN:" 
	spawn tcm_changeownerauth $opt -r -l debug
	sleep 1
	expect "Enter owner password" {
		exp_send "$ownerPasswd\r"
		if {$opt == "-o"} {
	...
	# Well Known -> Unicode
	puts stdout "Test 4) tcm_changeownerauth \"$opt\": Changing password from WELL KNOWN to UNICODE:" 
	spawn tcm_changeownerauth $opt $wnOpt -n -l debug
	sleep 1
	if {$wnOpt != "-z"} {
	...
	# Unicode -> Well Known
	puts stdout "Test 5) tcm_changeownerauth \"$opt\": Changing password from UNICODE to WELL KNOWN:" 
	spawn tcm_changeownerauth $opt $isU -r -l debug
	sleep 1
	if {$wnOpt != "-z"} {
		expect "Enter owner password" {exp_send "$ownerPasswd\r"}
	}
	...
	puts stdout "Test 6) tcm_changeownerauth \"$opt\": Changing password from WELL KNOWN to PLAIN:"
	spawn tcm_changeownerauth $opt $wnOpt -l debug
	sleep 1
	if {$wnOpt != "-z"} {
		expect "Enter owner password" {exp_send "$ownerPasswd\r"}
	}
